...
*/
void bootLora() {
  if (at_send_check_response("+AT: OK", LORA_CMD_TIMEOUT, "AT\r\n") >= 0) {
    at_send_check_response("+ID: DevAddr", LORA_CMD_TIMEOUT, "AT+ID\r\n");
    at_send_check_response("+MODE: LWOTAA", LORA_CMD_TIMEOUT, "AT+MODE=LWOTAA\r\n");
    at_send_check_response("+DR: EU868", LORA_CMD_TIMEOUT, "AT+DR=EU868\r\n");
    at_send_check_response("+CH: NUM", LORA_CMD_TIMEOUT, "AT+CH=NUM,0-2\r\n");
    at_send_check_response("+KEY: APPKEY", LORA_CMD_TIMEOUT, "AT+KEY=APPKEY,\"2B7E151628AED2A6ABF7158809CF4F3C\"\r\n");
    at_send_check_response("+CLASS: A", LORA_CMD_TIMEOUT, "AT+CLASS=A\r\n");
    at_send_check_response("+PORT: 8", LORA_CMD_TIMEOUT, "AT+PORT=8\r\n");
    at_send_check_response("+LOG: WARN", LORA_CMD_TIMEOUT, "AT+LOG=WARN");
...
void joinLoraNetwork() {
  ...
  int joinStatus = (at_send_check_response("Network joined", LORA_CMD_TIMEOUT, "AT+JOIN\r\n"));
  if (joinStatus >= 0) {
    hasLoraJoined = true;
...
void setup() {
  ...
  // Check if Wifi settings have to be erased
  if ((analogRead(WIFI_RESET_PIN)) > 1000) {
    Serial.println("Reseting Wifi settings");
    wifiManager.resetSettings();
  } else {
    Serial.println("Continuing with existing WiFi settings");
  }
...
rebootDiff = ((millis() / 1000) - REBOOT_AFTER_SEC);
  ...
  */
  ...
  if ((!(value[4])) && (!(value[5])) && (!(value[0])) && (!(value[1]))) {
    ...
    // loraSentMessage will be 1 if command is successfully executed
    loraSentMessage = at_send_check_response("Done", LORA_CMD_TIMEOUT, cmd);
    delay(WAIT_BETWEEN_READING);
    }

    /*
    ------------------------------trying to send payload via Wifi if unsuccessful from LoRa --------------------------------
    */
    if (WiFi.status() == WL_CONNECTED && (loraSentMessage != 1) ) {
      WiFiClient client1;
      HTTPClient http;
      ...
      debug("Couldn't send data from WiFi");
    }
  }
  /*
